     ...
     char **av;
{
    ...
    if (nwin_options.aka) {
        ...
        if (nwin_options.encoding > 0) {
            ...
            if (!newbuf)
                Panic(0, "%s", strnomem);
            newsz = RecodeBuf(nwin_options.aka, len, nwin_options.encoding, 0, newbuf);
            newbuf[newsz] = '\0';
  ...
  { \
...
#endif
    {
        ...
        if (SockDir) {
            ...
            SockDir = SOCKDIR;
            if (stat(SockDir, &st)) {
                n = (eff_uid == 0 && (real_uid || eff_gid == real_gid)) ? 0755 : (eff_gid != real_gid) ? 0775 :
#ifdef S_ISVTX
                    0777 | S_ISVTX;
#else
                    0777;
        ...
        case 'h':
            ...
            }
            {
                ...
                *p = 0;
                ...
                winmsg_numrend = -winmsg_numrend;
                MakeWinMsgEv(*s == 'h' ? win->w_hstatus : runbacktick(bt, &oldtick, now.tv_sec), win, '\005', 0, (struct event *) 0,
                             rec + 1);
        ...
        case 'W':
            {
                ...
                ss = AddWindows(p, l - 1, (*s == 'w' ? 0 : 1) | (longflg ? 0 : 2) | (plusflg ? 4 : 0) | (minusflg ? 8 : 0),
                                win ? win->w_number : -1);
                if (display)
                    D_fore = oldfore;
            }
    ...
    for (display = displays; display; display = display->d_next) {
        ...
        for (cv = D_cvlist; cv; cv = cv->c_next) {
            ...
            if (ly + cv->c_yoff < cv->c_ys) {
                ...
                CV_CALL(cv,
                        LScrollV(flayer, -n, 0, flayer->l_height - 1, 0); LayRedisplayLine(-1, -1, -1, 1); for (i = 0; i < n; i++)
                        LayRedisplayLine(i, 0, flayer->l_width - 1, 1); if (cv == cv->c_display->d_forecv)
                        LaySetCursor(););
                ...
                CV_CALL(cv,
                        ...
                        LayRedisplayLine(i + flayer->l_height - n, 0, flayer->l_width - 1, 1); if (cv == cv->c_display->d_forecv)
                        LaySetCursor(););
            }
            if (lx + cv->c_xoff < cv->c_xs) {
                ...
                CV_CALL(cv, LayRedisplayLine(-1, -1, -1, 1); for (i = 0; i < flayer->l_height; i++) {
                        LScrollH(flayer, -n, i, 0, flayer->l_width - 1, 0, 0); LayRedisplayLine(i, 0, n - 1, 1);}
